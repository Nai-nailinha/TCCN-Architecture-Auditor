...
def generate_markdown_report(result: AuditResult) -> str:
    lines: list[str] = [
        ...
        f"- Informações: {result.information_count}",
    ]
    ...
    lines.extend(_section("Documentos órfãos que exigem revisão"))

    if result.orphan_documents:
        for title in result.orphan_documents:
            lines.append(f"- {title}")
    else:
        lines.append("Nenhum documento órfão exige revisão.")

    lines.extend(_section("Documentos não referenciados permitidos"))

    if result.permitted_orphan_documents:
        for item in result.permitted_orphan_documents:
            lines.append(
                f"- **{item['document']}** — {item['reason']}."
            )
    else:
        lines.append("Nenhum documento foi dispensado pela política semântica.")
