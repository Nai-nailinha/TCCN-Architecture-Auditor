...
class AuditConfig:
    ...
    # Regras semânticas para documentos que podem existir sem referência de entrada.
    orphan_exempt_document_types: list[str] = field(default_factory=list)
    orphan_exempt_categories: list[str] = field(default_factory=list)
    orphan_exempt_paths: list[str] = field(default_factory=list)

    # Regras semânticas para artefatos que não usam o cabeçalho completo
    # dos documentos arquiteturais oficiais.
    metadata_exempt_document_types: list[str] = field(default_factory=list)
    metadata_exempt_categories: list[str] = field(default_factory=list)
    metadata_exempt_paths: list[str] = field(default_factory=list)
...
class AuditResult:
    ...
    permitted_orphan_documents: list[dict[str, str]] = field(default_factory=list)
    ...
    def information_count(self) -> int:
        return len(self.permitted_orphan_documents)
